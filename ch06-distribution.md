...
[^20]: *Community for Creative Non-Violence v. Reid,* 490 U.S. 730 (1989)에서 저작물이 (회사가 소유한) 직무저작물인지 혹은 (회사가 소유한 것이 아닌 저자가 소유) 아닌지 결정하는 요인은 다른 것중에서 저작물을 생성하는데 요구되는 기술 수준, 저작물을 생성하는데 사용된 도구의 원천, 고객과 저자간에 관계를 맺은 기간, 언제 얼마나 오랜동안 작업하는데 도급업자의 재량권 정도, 그리고 저작물이 고객 혹은 컨설턴트의 정규 사업 부분인지 여부다. 따라서, 많은 자문 합의서는 작업이 어디서 수행되었을 뿐만 아니라 배포가 일어난 것과 연관된 다른 사실도 나열한다.

#### 인수 합병

- **GPL 소프트웨어 전달을 피하라.** 특히 자산매입거래에서, 매입자가 원래 소스코드 혹은 제3자 소스코드에서 직접 다운로드하도록 오픈소스 팩키지 전달을 회피하여 삼갈 수 있는 합리적인 방법이 있는지 알아내라. 판매자에게 소속된 드라이버 혹은 다른 중요한 원본 코드가 전달되는 상황에서 대체로 이런 접근법은 유용하다. 판매자가 통합된 변경사항을 전달할 때는 유용하지 않는다. 이런 경우, 판매자는 판매자의 추가분만 전달하고 구매자는 제3자 오픈소스 코드를 별도로 받는다. 분명히, 만약 제3자 오픈소스 코드가 광범위하게 변경되면, 이런 전략은 그다지 실현가능성이 없는데 이유는 판매자 코드와 제3자 코드를 나누는 것이 너무 어렵기 때문이다. 하지만, 이런 분쟁에 있어 매우 보수적인 회사는 어떤 제3자 GPL 코드 전달을 회피하려는 시도의 일환으로, 단지 *차이분*, 혹은 패치만 전달한다. 배포는 대체로 구매자가 아닌 판매자에 대한 분쟁임을 명심하라. 따라서, 판매자 법인의 모든 자산으로 구성된 자산 매입에 대한 걱정은 가능성이 적어 고려할 가치가 없다. 하지만, 판매자의 일부 자산, 사업 분야, 제품군 매각은 GPL 배포에 관한 걱정을 판매자가 갖도록 할 수 있다. 만약 코드가 GPL 아래 전달되야 한다면, 본인 코드를 판매하려는 판매자는 해당 코드에 대해서 지불하기 꺼려하는 구매자를 발견할 수도 있다.

#### SAAS 합의서

- **SAAS와 배포를 혼동하는 문서작업을 피하라.** 기술 변호사 사이에 SAAS 합의서가 라이선스인가 혹은 단순히 서비스 합의서인가에 대해서 일부 논쟁이 있다. 종종, 분산된 소프트웨어에서 사업 선행사건의 산출물로, SAAS 합의서가 흡사 분산 소프트웨어 라이선스처럼 작성되어서 둘을 분간하기는 어렵다. 설사 배포질문이 그저 문서 작성이 아닌 주로 공급자의 행동에 달려있지만, 분산 제품을 다루는 것처럼 읽히는 SAAS 합의서를 사용함으로써 본인 위치를 손상시키지 않는 것이 최선이다.

#### 회사간 합의서(Intercompany Agreements)

- **배포하지 않는 의도를 나열하라.** 제휴사 사이 소프트웨어 합의서에서, 컨설팅 혹은 개발합의서에 대해서 앞에서 권장한 방식과 동일하게 부모 법인체는 어떤 배포도 의도되지 않았다고 명확히 하고자 한다. 이점이 명확한 것 같지만 사실 회사간 기술 라이선스는 기술 변호사에 의해서 문서 작업이 종종 되지 않는다. 대신에, 지적재산 분쟁을 정밀하게 고려하기 보다는 귀속 세금 분쟁에 대처할 목적으로 회사간 계약을 문서화하는 세금 변호사 혹은 기업 변호사가 초안 작업을 한다. 오픈소스 분쟁 뿐만 아니라 지적재산 분쟁에 대한 관점을 갖고 합의서를 검토하는 것이 매우 중요하다.

### 지속되는 수수께끼

미국 연방법원이 조만간 배포 질문에 대답을 제시할 것 같지는 않다. 지금까지 가져온 오픈소스 시행법령은 이런 질문을 다루지 않았다. 
(GPL2 아래 2차 저작물 범위와 특허법과 오픈소스 라이선싱의 상호작용 같은) 다른 자극적인 쟁점이 오픈소스 법에서 여전히 명확하지 않은 상황에서, 논쟁은 아직 무르익지 않았다. 또한, GPL2 아래 코드를 출시하는 저자 대부분은 회사간 합의서와 인수 혹은 합병 같은 쟁점에 단순히 초점을 맞추지 않고 있다. 이유는 주로 저자들이 기업 전략가라기 보다는 기술자이기 때문이다. 만약 GPL 저자가 일반적으로 극단적 경우에 자신의 권리를 시행하려는 의도는 없지만, 이 영역에 법을 제정하는 소송을 제기하는데 관심을 갖는 유권자도 아닐 수 있다. 따라서, GPL2가 폭넓게 사용되는 라이선스라면 이런 질문이 지속될 것 같다. 그리고, 리눅스 커널 자체 유행으로 장기간 지속될 것이다. 오픈소스 준수를 사정하는 회사는 가장 문제가 될 것 같은 배포유형을 확실히 식별해서 확신을 갖고 오픈소스 소프트웨어를 사용하고 오픈소스 준수 전략에 맞추는 방식으로 거래를 준비한다.
